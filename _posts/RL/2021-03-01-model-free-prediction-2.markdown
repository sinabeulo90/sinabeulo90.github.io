...
- TD: R_{t+1} + \gamma V(S_{t+1}) 방향으로 업데이트 한다.
	...
	- Ex: 차를 운전하고 있는데, 실수로 중앙선을 넘었다고 하자. 그런데 건너편에서 트럭이 달려오고 있을 때, 트럭의 반응속도가 좋아서 비켜 지나가고 나는 다시 원래 차선으로 돌아온 상황이라고 하자.
		...
		- TD의 경우: 중앙선을 침범하는 순간, 다음 step의 상태에서 내가 죽을 것 같다는 예측을 할 수 있기 때문에, 다시 중앙선으로 돌아왔을 때 그 상황에 대해 추측을 사용해서 update를 한다.
			- 한 step만큼 가서 그 step에서 추측을 하는 것: V(s_{t+1})
			- 그 과정에서 받은 Reward: R_{t+1}
			- step만큼 가서 예측을 한 것: R_{t+1} + \gamma V(S_{t+1})
				- 이전의 예측치보다 정확할 것이다. 왜냐하면 한 step만큼 현심이 더 반영된 것이기 때문이다. 이 값으로 현재 V(s_t)를 업데이트 한다.
...
- True TD Target: 전지전능한 신이 V_\pi(S_{t+1})의 실제 값을 알려주면, R_{t+1} + \gamma V_\pi(S_{t+1})은 편향되지 않는 값이다.
	- 왜냐하면 Bellan Equation이 이를 보장하기 때문
- 그런데 우리는 V_\pi(S_{t+1})의 실제 값을 모르기 때문에, TD target R_{t+1} + \gamma V_\pi(S_{t+1})은 편향되어 있는 값이다.
...
- Model free이기 때문에 왜 이렇게 되는지 규칙을 알 수 없다.
...
- Sampling: Full sweep하지 않고, 하나의 대해서 샘플을 가지고 업데이트
	- MC, TD o: 너비 관점에서, 해본 샘플들을 가지고 업데이트 하기 때문
		- MC가 끝까지 가던, TD는 한 스텝만 하던, 했던 것을 가지고 업데이트 하기 때문
...
- 에피소드가 10개로 제한되어 있기 때문에, MC return의 variance 크기 때문에 성능이 좋지 않다.
