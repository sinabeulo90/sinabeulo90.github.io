...
- 단점
    - 바로 배우는 것이 아니라, 먼저 모델을 배우고 그 모델로부터 value와 policy의 최적을 찾아나간다. 즉 2단계를 거쳐서 학습이 진행되기 때문에 틀릴 수 있는 군데가 2군데나 생긴다.
...
- state A, B 2개가 있고 왼쪽과 같은 어떤 경험들이 있을 때, 이 경험을 바탕으로 모델이 어떻게 될까?
    ...
    - B는 8번중 6번은 1로, 2번은 0으로 가기 때문에 각각 75%, 25% 확률로 terminal state에 도착한다.
